...
const getTaskData = () => {
    const taskID = Number(route.params.projectID) + Number(route.params.taskID)
    sendAuthRequest({ url: `/categories/${route.params.id}/projects/${route.params.projectID}/tasks/${taskID}`, method: 'get' })
        .then((response) => {
            ...
            if (response.status === 200) {
                ...
                if (task.value._tasks) {
                    ...
                    task.value._tasks.forEach((task) => {
                        task.id = task.id - route.params.projectID
                        task._href = `${task.id}`
                    })
                }
            }
